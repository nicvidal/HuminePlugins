...
public class MenuList {
	...
	public static Menu noKingdomMenu(){
		...
		declineM.setDisplayName(ChatColor.YELLOW+"Non une autre fois peut-être..."+ChatColor.RED+" X");
	...
	public static Menu MemberMenu(Kingdom k){
		...
		File member = new File("./plugins/HumineKingdomV2/Kingdom/"+k.getName()+"/member.yml");
		int slot = 0;
	...
	public static Menu BeaconList(Kingdom k){
		...
		for(Zone zone : zones){
			...
			balisem.setDisplayName(ChatColor.DARK_BLUE+""+zone.getLocation().getBlockX()+ChatColor.WHITE+" - "+ChatColor.DARK_BLUE+zone.getLocation().getBlockZ());
