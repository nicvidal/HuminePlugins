	...
	public void addNewPerms(String perm){
		...
		for(OfflinePlayer op : pls){
			if(op.isOnline()){
				...
				GradeManager.addPerm(op.getPlayer(), perm, kingdom);
			}
		}
	...
	public void addNewPlayer(OfflinePlayer p, boolean register){
		...
		if(p.isOnline()){
			for(String perm : perms){
				...
				GradeManager.addPerm(p.getPlayer(), perm, kingdom);
			}
		}
		if(register == true){
			...
			try(PrintWriter print = new PrintWriter(new FileOutputStream(HumineKingdom.getDataKingdom(), true))){
				...
				print.println(p.getName()+" = "+kingdom.getName()+" = "+name+" = "+point);
				System.out.println(p.getName()+" = "+kingdom.getName()+" = "+name+" = "+point);
			}catch(IOException e){
				e.printStackTrace();
			}
			...
			for(String str : tempo){
				...
				if(args[0].equalsIgnoreCase(offlinePlayer.getName())){
					if(HumineKingdom.getPlayerkingdom(offlinePlayer) != null)
						print.println(offlinePlayer.getName() + " = "+HumineKingdom.getPlayerkingdom(offlinePlayer)+" = aucun = " + args[3]);
					else
